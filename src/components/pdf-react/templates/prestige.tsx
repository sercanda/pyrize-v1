...
const s = StyleSheet.create({
  page: { fontFamily: 'Poppins', fontSize: 10, color: c.text, backgroundColor: c.cream, padding: 0 },
  hero: { backgroundColor: c.navy, padding: 50, color: c.white, alignItems: 'center' },
  heroTitle: { fontSize: 28, fontWeight: 700, color: c.gold, textAlign: 'center', marginBottom: 10, letterSpacing: 1 },
  heroSub: { fontSize: 11, color: c.white, opacity: 0.85, textAlign: 'center' },
  divider: { width: 80, height: 1, backgroundColor: c.gold, marginVertical: 15, alignSelf: 'center' },
  section: { padding: '15 50' },
  heading: { fontSize: 14, fontWeight: 700, color: c.navy, marginBottom: 10, letterSpacing: 0.5 },
  card: { backgroundColor: c.white, borderRadius: 4, padding: 14, marginBottom: 8 },
  cardTitle: { fontSize: 10, fontWeight: 600, color: c.navy, marginBottom: 3 },
  cardText: { fontSize: 9, color: c.textLight, lineHeight: 1.5 },
  grid: { flexDirection: 'row', gap: 10 },
  stat: { flex: 1, backgroundColor: c.white, padding: 14, alignItems: 'center', borderTop: `2 solid ${c.gold}` },
  ...
  agentBox: { backgroundColor: c.navy, padding: 30, margin: '10 50', color: c.white, alignItems: 'center', borderRadius: 4 },
});

interface Props { istek: any; icerik: any; }

export default function PrestigePdf({ istek, icerik }: Props) {
    ...
    <Document>
      <Page size="A4" style={s.page}>
        <View style={s.hero}>
          ...
          <Text style={s.heroTitle}>{icerik.baslik || 'Prestij Gayrimenkul Sunumu'}</Text>
        ...
        <View style={[s.section, s.grid]}>
          ...
          {val?.estimatedValueRange && <View style={s.stat}><Text style={s.statLabel}>DEGER ARALIGI</Text><Text style={s.statValue}>{val.estimatedValueRange}</Text></View>}
        </View>
        <Text style={s.pageNum}>1 / 6</Text>
      </Page>

      <Page size="A4" style={s.page}>
        <View style={s.section}><Text style={s.heading}>Piyasa Degerlendirmesi</Text></View>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.section}><Text style={s.heading}>Ozel Hizmet Paketimiz</Text></View>
        <View style={[s.grid, { padding: '0 50' }]}>
          {solutions.map((sol: any, i: number) => (
            <View key={i} style={[s.card, { flex: 1, borderTop: `2 solid ${c.gold}` }]}>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.section}><Text style={s.heading}>Bolge ve Hedef Kitle Analizi</Text></View>
        <View style={{ padding: '0 50' }}>
          <Text style={s.cardText}>{find('location_analysis')?.icerik || 'Detayli konum analizi.'}</Text>
        </View>
        {find('target_audience')?.altBolge?.slice(0, 3).map((t: any, i: number) => (
          <View key={i} style={[s.card, { marginHorizontal: 50, marginTop: 6 }]}>
      ...
      <Page size="A4" style={s.page}>
        <View style={s.section}><Text style={s.heading}>Taahhutlerimiz</Text></View>
        <View style={[s.grid, { padding: '0 50' }]}>
          {(guarantees.length > 0 ? guarantees : [
            ...
            <View key={i} style={s.stat}>
              <Text style={[s.statLabel, { fontWeight: 600, color: c.text }]}>{g.baslik}</Text>
              <Text style={[s.statLabel, { marginTop: 4 }]}>{g.icerik}</Text>
            </View>
          ))}
        </View>
        <View style={s.agentBox}>
          ...
          <Text style={{ fontSize: 9, color: c.white, opacity: 0.85, marginTop: 4 }}>Gayrimenkul Danismani</Text>
          <Text style={{ fontSize: 9, color: c.white, opacity: 0.85, marginTop: 4 }}>{danisman.telefon} - {danisman.email}</Text>
        </View>
        <View style={s.footer}><Text>{new Date().getFullYear()} {danisman.adSoyad} - Prestij Sunum</Text></View>
