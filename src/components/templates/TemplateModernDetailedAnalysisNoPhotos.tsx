...
const mapDataToTemplate = (data: OlusturulanSunum) => {
  ...
  const consultant: Consultant = {
    ...
    profilFotografiUrl:
      ((danisman as any)?.profilFotografi && (danisman as any).profilFotografi.trim().length > 0)
        ? (danisman as any).profilFotografi
        : ((danisman as any)?.profilFotografiUrl && (danisman as any).profilFotografiUrl.trim().length > 0)
          ? (danisman as any).profilFotografiUrl
          : 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=400',
    ofisLogosuUrl:
      ((danisman as any)?.ofisLogosu && (danisman as any).ofisLogosu.trim().length > 0)
        ? (danisman as any).ofisLogosu
        : ((danisman as any)?.ofisLogosuUrl && (danisman as any).ofisLogosuUrl.trim().length > 0)
          ? (danisman as any).ofisLogosuUrl
          : 'https://i.ibb.co/1yynDd7/e92f870139b241e9820965c4ac5167b3-removebg-preview.png',
  ...
  const property: Property = {
    gorselUrl: FALLBACK_IMAGE,
    planBaslik: baslik,
    planAltBaslik: altBaslik,
    konumAnalizi: {
...
const TemplateModernDetailedAnalysisNoPhotos: React.FC<{ data: OlusturulanSunum }> = ({ data }) => {
          ...
          <div className="min-h-screen flex flex-col print:block print:h-auto print:break-after-page print:px-8 print:py-8 relative">
            ...
            <nav className="w-full py-6 px-6 md:px-12 flex justify-between items-end border-b border-white/5 bg-slate-950 relative z-50 print:bg-white print:border-b-2 print:border-slate-900 print:px-0 print:py-4 print:static print:mb-12">
              ...
              <div className="text-right hidden md:block print:block">
                ...
                <p className="text-sm font-mono text-slate-300 print:text-black print:font-bold">
                  TR-{((mapped.property.konumAnalizi.ilIlce || 'LOC').replace(/\s+/g, '-').toUpperCase())}-{Date.now().toString().slice(-4)}
                </p>
              </div>
            </nav>
            ...
            <div className="flex-1 flex flex-col justify-center gap-12 px-6 md:px-12 lg:px-16 print:px-0 print:gap-8 print:block">
              ...
              <HeroSection
                ...
                heroDescription={data.icerik.heroAciklama}
                theme={THEME_MODERN}
              />
...
export default TemplateModernDetailedAnalysisNoPhotos;
